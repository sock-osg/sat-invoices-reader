...
class PayrollParser(InvoceParserInterface):
    ...
    def read_file(self,fileName, file):
        ...
        if cfdiUuid not in cfdiPrintedRegistry:
            ...
            print(fileName, file.get('version'), cfdiUuid.upper(), file.get('fecha').upper(), concept.get('descripcion'), nomina.get('fechainicialpago'), nomina.get('fechafinalpago'), emisor.get('nombre').upper(), emisor.get('rfc').upper(), file.get('subtotal'), paidTax, file.get('total'), sep='|')
